process_exercise_button_click <- function(token, db, payload) {
  ...
  if(payload$actions$action_id == "exercise_finished") {
    ...
    if (!payload$user$id %in% (tbl(db, from = "users") %>% collect() %>% .$user)) {
      ...
      if(res$ok) {
        dbWriteTable(db, "users", tibble::tibble(user = res$user$id, name = res$user$name, avatar = res$user$profile$image_24), append = T)
